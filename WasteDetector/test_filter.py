...
# Balanced German Waste Separation System (VerpackG)
# ============================================================
...
# ============================================================
# OFFICIAL GERMAN WASTE DEFINITIONS (DUALES SYSTEM)
# ============================================================
...
RESTMUELL_DEFAULT_RULE = {
    ...
    "hex": "#94A3B8",
    "tip": "Gegenstand nicht eindeutig zuordenbar. Im Zweifel Restmüll.",
    "group": "residual"
}
...
ALTGLAS_UNIFIED_RULE = {
    "bin": "Altglas (Grün / Braun / Weiß)",
    "color": (129, 185, 16),      # Emerald Green BGR (Distinct from Altpapier Blue!)
    "hex": "#10B981",
    "tip": "In den passenden Glascontainer werfen (Grün, Braun oder Weiß). Blaues Glas zu Grünglas!",
    "group": "glass"
}
...
PFAND_RULE = {
    ...
    "hex": "#F87171",
    "tip": "Getränkeverpackungen mit DPG-Logo im Supermarkt abgeben.",
    "group": "deposit"
}

ELEKTRO_RULE = {
    "bin": "Elektroschrott / Wertstoffhof",
    "color": (0, 0, 230),         # Red BGR
    "hex": "#F87171",
    "tip": "Elektronik & Batterien dürfen NIEMALS in den Hausmüll!",
    "group": "deposit"
}

# ============================================================
# GERMAN WASTE MAPPING TABLE
# ============================================================

GERMAN_WASTE_RULES = {
    # --------------------------------------------------------
    # 1. GELBER SACK / GELBE TONNE / WERTSTOFFTONNE (Yellow Bin)
    # --------------------------------------------------------
    ...
    "handbag": GELBER_SACK_RULE,
    "backpack": GELBER_SACK_RULE,
    "suitcase": GELBER_SACK_RULE,

    # --------------------------------------------------------
    # 2. ALTGLAS (Unified Category - Grün / Braun / Weiß)
    # --------------------------------------------------------
    "glass_container": ALTGLAS_UNIFIED_RULE,
    "glass_bottle": ALTGLAS_UNIFIED_RULE,
    "jar": ALTGLAS_UNIFIED_RULE,
    "bottle": ALTGLAS_UNIFIED_RULE,

    # TRINKGLÄSER & KERAMIK SIND RESTMÜLL (Not affected by glass rule!)
    "wine glass": {
        "bin": "Restmüll! (KEIN Altglas!)",
        "color": (70, 70, 70),
        "hex": "#F87171",
        "tip": "Trinkgläser gehören wegen abweichendem Schmelzpunkt in den Restmüll!",
        "group": "residual"
    },
    ...
    "pizza": {
        "bin": "Biomüll (Speisereste)",
        "color": (42, 42, 165),
        "hex": "#34D399",
        "tip": "Essensreste in Biomüll, fettigen Karton in Restmüll!",
        "group": "organic"
    },
    ...
    "toothbrush": {"bin": "Restmüll (Graue Tonne)", "color": (60, 60, 60), "hex": "#94A3B8", "tip": "Alte Zahnbürsten gehören in den Restmüll.", "group": "residual"},
...
# Balanced Keyword Lists
ORGANIC_KEYWORDS = ["banana", "apple", "sandwich", "orange", "food", "fruit", "vegetable"]
PAPER_KEYWORDS = ["paper", "cardboard", "box", "carton", "book"]
GLASS_KEYWORDS = ["glass", "jar"]
PLASTIC_KEYWORDS = ["plastic", "plastico", "pet", "poly", "wrapper", "chips", "foil", "tetra", "pack"]
ELEKTRO_KEYWORDS = ["phone", "remote", "mouse", "keyboard", "battery", "laptop", "cable"]
...
def load_detection_model():
    """
    Loads custom model if available. Fallback exports YOLOv10s/v8s to ONNX format
    for fast CPU performance on AMD Ryzen processors.
    """
...
def match_waste_rule(class_name):
    """
    Evaluates detected object against waste sorting rules.
    Strictly defaults to Restmüll if an item is not recognized.
    """
    ...
    # 2. Balanced Keyword Priority Chain
    if any(k in class_name for k in ORGANIC_KEYWORDS):
        return BIOMUELL_RULE

    if any(k in class_name for k in PAPER_KEYWORDS):
    ...
    if any(k in class_name for k in ELEKTRO_KEYWORDS):
        return ELEKTRO_RULE

    # 3. Substring Key Match
    for key, rule in GERMAN_WASTE_RULES.items():
        if key in class_name or class_name in key:
            return rule

    # 4. STRICT DEFAULT: RESTMÜLL (GRAUE TONNE)
...
def scanning():
    ...
    if held_object is not None:
        ...
        rule = match_waste_rule(class_name)
        ...
        # Draw classification label above bounding box
